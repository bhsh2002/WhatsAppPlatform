...
const FacebookInsights = () => {
  ...
  const loadDaily = useCallback(async () => {
    ...
    try {
      ...
      const data = await api.getFbPageDaily(selectedPageId, {
        since,
        until
      });
      setDaily(data.daily || []);
  ...
  const loadPosts = useCallback(async () => {
    ...
    try {
      ...
      const data = await api.getFbPostInsights(selectedPageId, {
        limit: 10
      });
      setPosts(data.posts || []);
        ...
        alignItems: 'center'
          ...
          minWidth: 250
                        ...
                        <Select value={selectedPageId} onChange={e => setSelectedPageId(e.target.value)} label={tx("auto.k_338c505c00b5")}>
                            {allPages.length === 0 ? <MenuItem value="" disabled>{tx("auto.k_43a46f260ab6")}</MenuItem> : allPages.map(page => <MenuItem key={page.id} value={page.id}>
                                        {page.page_name || page.page_id}
                                        {page.tenant_name && <Chip label={page.tenant_name} size="small" sx={{
                ml: 1
              }} />}
                                    </MenuItem>)}
                        </Select>
