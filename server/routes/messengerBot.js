...
function normalizeFlowPayload(body = {}) {
    ...
    const nodeType = VALID_NODE_TYPES.has(node.node_type || body.node_type) ? (node.node_type || body.node_type) : 'text';
    ...
    return {
        ...
        node: {
            node_key: String(node.node_key || 'start').trim() || 'start',
            node_type: nodeType,
            title: String(node.title || body.title || '').trim() || null,
            body: String(node.body || body.body || '').trim() || null,
            config_json: serializeJson(node.config || parseJson(node.config_json, {})),
            sort_order: toInt(node.sort_order, 0),
        },
    };
}
...
function upsertStartNode(flowId, node) {
    db.prepare(`
    ...
    `).run(flowId, node.node_key, node.node_type, node.title, node.body, node.config_json, node.sort_order);
}
...
router.post('/flows', (req, res) => {
    try {
        const tenant = requireTenant(req, res);
        if (!tenant) return;
        const { flow, node } = normalizeFlowPayload(req.body);
        ...
        const tx = db.transaction(() => {
            ...
            upsertStartNode(result.lastInsertRowid, node);
            return getFlowWithNode(result.lastInsertRowid, tenant.id);
        });
...
router.put('/flows/:id', (req, res) => {
    try {
        ...
        const { flow, node } = normalizeFlowPayload({ ...existing, node: existing.node, ...req.body });
        if (!flow.name) return res.status(400).json({ error: 'اسم المسار مطلوب' });
        if (flow.linked_page_id && !validateLinkedPage(tenant.id, flow.linked_page_id)) {
            return res.status(400).json({ error: 'صفحة فيسبوك غير صالحة لهذا العميل' });
        }
        ...
        const tx = db.transaction(() => {
            ...
            upsertStartNode(req.params.id, node);
            return getFlowWithNode(req.params.id, tenant.id);
        });
...
router.patch('/flows/:id/toggle', (req, res) => {
    try {
        ...
        res.json(getFlowWithNode(req.params.id, tenant.id));
    } catch (error) {
