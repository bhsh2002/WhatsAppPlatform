...
import {
    ...
    Divider, Grid, Select, InputLabel, FormControl, RadioGroup, Radio,
    Table, TableBody, TableCell, TableContainer, TableHead, TableRow,
    Tooltip
} from '@mui/material';
import {
    Add as AddIcon, Edit as EditIcon, Delete as DeleteIcon,
    SmartToy as SmartToyIcon, VpnKey as KeywordIcon,
    WavingHand as WelcomeIcon, NightsStay as AwayIcon,
    PlayArrow as TestIcon, Refresh as RefreshIcon,
    CheckCircle as CheckCircleIcon, Cancel as CancelIcon,
    Science as ScienceIcon, ChatBubble as CommentReplyIcon,
    Facebook as FBPageIcon, WhatsApp as WhatsAppIcon
...
const DAY_OPTIONS = [
    { value: 'sun', label: 'أحد' }, { value: 'mon', label: 'اثنين' },
    { value: 'tue', label: 'ثلاثاء' }, { value: 'wed', label: 'أربعاء' },
    { value: 'thu', label: 'خميس' }, { value: 'fri', label: 'جمعة' },
    { value: 'sat', label: 'سبت' },
];
...
const emptyRule = {
    name: '', rule_type: 'keyword', channel: 'all',
    priority: 100, is_active: true,
    match_type: 'contains', match_pattern: '', match_case_sensitive: false,
    schedule_days: ['sun', 'mon', 'tue', 'wed', 'thu'],
    schedule_start_time: '20:00', schedule_end_time: '08:00',
    schedule_timezone: 'Africa/Tripoli',
    response_type: 'text', response_text: '',
    cooldown_seconds: 300,
    target_post_id: '', target_page_id: '',
    response_action: 'comment', dm_text: '', trigger_on: 'comment',
    auto_like: false, auto_like_type: 'like',
};
...
const TenantAutomation = () => {
    ...
    const handleOpenEdit = (rule) => {
        setEditingRule(rule);
        let scheduleDays = [];
        try { scheduleDays = rule.schedule_days ? JSON.parse(rule.schedule_days) : []; } catch { scheduleDays = []; }
        setFormData({
            name: rule.name || '', rule_type: rule.rule_type || 'keyword',
            channel: rule.channel || 'all', priority: rule.priority || 100,
            is_active: !!rule.is_active, match_type: rule.match_type || 'contains',
            match_pattern: rule.match_pattern || '', match_case_sensitive: !!rule.match_case_sensitive,
            schedule_days: scheduleDays, schedule_start_time: rule.schedule_start_time || '20:00',
            schedule_end_time: rule.schedule_end_time || '08:00', schedule_timezone: rule.schedule_timezone || 'Africa/Tripoli',
            response_type: rule.response_type || 'text', response_text: rule.response_text || '',
            cooldown_seconds: rule.cooldown_seconds ?? 300,
            target_post_id: rule.target_post_id || '', target_page_id: rule.target_page_id || '',
            response_action: rule.response_action || 'comment', dm_text: rule.dm_text || '',
            trigger_on: rule.trigger_on || 'comment', auto_like: !!rule.auto_like,
    ...
    const getChannelChip = (ch) => {
        ...
        if (ch === 'messenger') return <Chip icon={<FBPageIcon />} label="ماسنجر" size="small" sx={{ bgcolor: '#0084ff22', color: '#0084ff' }} />;
        ...
        <Box sx={{ p: { xs: 1.5, md: 3 }, maxWidth: 1200, mx: 'auto' }}>
                        ...
                        <TableBody>
                            {rules.map(rule => (
                                <TableRow key={rule.id} sx={{ '&:hover': { bgcolor: 'grey.50' } }}>
                                    <TableCell><Chip label={rule.priority} size="small" variant="outlined" sx={{ fontWeight: 'bold' }} /></TableCell>
                                    ...
                                    <TableCell>
                                        <Tooltip title="تعديل"><IconButton size="small" onClick={() => handleOpenEdit(rule)}><EditIcon fontSize="small" /></IconButton></Tooltip>
                                        <Tooltip title="حذف"><IconButton size="small" color="error" onClick={() => setDeleteConfirm(rule)}><DeleteIcon fontSize="small" /></IconButton></Tooltip>
                                    </TableCell>
                                </TableRow>
                            ))}
                        </TableBody>
            ...
            <Paper sx={{ p: 3, mt: 3 }}>
                ...
                <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', alignItems: 'flex-end' }}>
                    ...
                    <TextField size="small" label="نص الرسالة" value={testMessage} onChange={e => setTestMessage(e.target.value)} sx={{ flex: 1, minWidth: 200 }} />
                    <FormControlLabel control={<Checkbox checked={testNewContact} onChange={e => setTestNewContact(e.target.checked)} size="small" />} label="جهة اتصال جديدة" />
                    <Button variant="contained" startIcon={testing ? <CircularProgress size={16} color="inherit" /> : <TestIcon />} onClick={handleTest} disabled={testing || (!testMessage && !testNewContact)}>
                        اختبار
                    </Button>
                </Box>
                {testResult && (
                    <Box sx={{ mt: 2 }}>
                        {testResult.error ? (
                            ...
                            <Alert severity="success" icon={<CheckCircleIcon />}>
                                <Typography variant="subtitle2">ستطابق: "{testResult.rule.name}" (أولوية {testResult.rule.priority})</Typography>
                                <Typography variant="body2" sx={{ mt: 0.5, opacity: 0.8 }}>
                                    الرد: {testResult.response_text?.substring(0, 150)}{testResult.response_text?.length > 150 ? '...' : ''}
                                </Typography>
                            </Alert>
                        ) : (
                            <Alert severity="info" icon={<CancelIcon />}>لا توجد قاعدة مطابقة لهذه الرسالة</Alert>
                        )}
                    </Box>
                )}
            </Paper>
            ...
            <Dialog open={dialogOpen} onClose={() => !saving && setDialogOpen(false)} maxWidth="sm" fullWidth>
                <DialogTitle>{editingRule ? `تعديل: ${editingRule.name}` : 'إنشاء قاعدة جديدة'}</DialogTitle>
                <DialogContent dividers>
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: 1 }}>
                        <TextField label="اسم القاعدة" value={formData.name} onChange={e => setFormData(p => ({ ...p, name: e.target.value }))} required fullWidth />

                        <Box sx={{ display: 'flex', gap: 2 }}>
                            <FormControl fullWidth>
                                <InputLabel>نوع القاعدة</InputLabel>
                                <Select value={formData.rule_type} onChange={e => setFormData(p => ({ ...p, rule_type: e.target.value }))} label="نوع القاعدة" disabled={!!editingRule}>
                                    {RULE_TYPES.map(t => (
                            ...
                            <FormControl fullWidth>
                                <InputLabel>القناة</InputLabel>
                                <Select value={formData.channel} onChange={e => setFormData(p => ({ ...p, channel: e.target.value }))} label="القناة">
                                    {CHANNELS.map(c => <MenuItem key={c.value} value={c.value}>{c.label}</MenuItem>)}
                                </Select>
                            </FormControl>
                        </Box>

                        <TextField label="الأولوية" type="number" value={formData.priority} onChange={e => setFormData(p => ({ ...p, priority: parseInt(e.target.value) || 100 }))} helperText="أقل = أعلى أولوية" sx={{ minWidth: 120 }} />
                            ...
                            <>
                                ...
                                <Box sx={{ display: 'flex', gap: 2 }}>
                                    <FormControl sx={{ minWidth: 160 }}>
                                        ...
                                        <Select value={formData.match_type} onChange={e => setFormData(p => ({ ...p, match_type: e.target.value }))} label="نوع المطابقة">
                                            {MATCH_TYPES.map(m => <MenuItem key={m.value} value={m.value}>{m.label}</MenuItem>)}
                                        </Select>
                                    </FormControl>
                                    <TextField label={formData.match_type === 'contains' ? 'الكلمات (فاصلة بين كل كلمة)' : 'النمط'} value={formData.match_pattern} onChange={e => setFormData(p => ({ ...p, match_pattern: e.target.value }))} fullWidth placeholder={formData.match_type === 'contains' ? 'سعر,أسعار,كم السعر' : ''} />
                                </Box>
                                <FormControlLabel control={<Checkbox checked={formData.match_case_sensitive} onChange={e => setFormData(p => ({ ...p, match_case_sensitive: e.target.checked }))} />} label="حساسية لحالة الأحرف" />
                            </>
                        )}

                        {formData.rule_type === 'welcome' && (
                            <Alert severity="info">سيتم إرسال هذا الرد تلقائياً عند استقبال أول رسالة من جهة اتصال جديدة</Alert>
                        )}
                        ...
                        {formData.rule_type === 'away' && (
                            <>
                                ...
                                <FormGroup row>
                                    {DAY_OPTIONS.map(day => (
                                        <FormControlLabel key={day.value} control={<Checkbox checked={formData.schedule_days.includes(day.value)} onChange={e => {
                                            setFormData(p => ({ ...p, schedule_days: e.target.checked ? [...p.schedule_days, day.value] : p.schedule_days.filter(d => d !== day.value) }));
                                        }} size="small" />} label={day.label} />
                                    ))}
                                </FormGroup>
                                <Box sx={{ display: 'flex', gap: 2 }}>
                                    <TextField label="بداية (خارج الدوام)" type="time" value={formData.schedule_start_time} onChange={e => setFormData(p => ({ ...p, schedule_start_time: e.target.value }))} InputLabelProps={{ shrink: true }} fullWidth />
                                    <TextField label="نهاية (خارج الدوام)" type="time" value={formData.schedule_end_time} onChange={e => setFormData(p => ({ ...p, schedule_end_time: e.target.value }))} InputLabelProps={{ shrink: true }} fullWidth />
                                </Box>
                                <Alert severity="info" sx={{ fontSize: '0.8rem' }}>مثال: 20:00 إلى 08:00 = الرد يعمل من الثامنة مساءً إلى الثامنة صباحاً</Alert>
                            </>
                        )}
                            ...
                            <>
                                ...
                                <Box sx={{ display: 'flex', gap: 2 }}>
                                    ...
                                    <FormControl fullWidth>
                                        ...
                                        <Select value={formData.target_post_id} onChange={e => setFormData(p => ({ ...p, target_post_id: e.target.value }))} label="المنشور" disabled={!formData.target_page_id || postsLoading}>
                                            <MenuItem value="">جميع المنشورات</MenuItem>
                                            {fbPosts.map(post => (<MenuItem key={post.id} value={post.id}>{(post.message || 'بدون نص').substring(0, 60)}{(post.message || '').length > 60 ? '...' : ''}</MenuItem>))}
                                        </Select>
                                    </FormControl>
                                </Box>
                                ...
                                <Box sx={{ display: 'flex', gap: 2 }}>
                                    ...
                                    <TextField label="الكلمات (اتركه فارغاً للرد على الكل)" value={formData.match_pattern} onChange={e => setFormData(p => ({ ...p, match_pattern: e.target.value }))} fullWidth placeholder="مثال: سعر,تفاصيل,كم" />
                                </Box>
                                ...
                                <RadioGroup row value={formData.trigger_on} onChange={e => {
                                    ...
                                    setFormData(p => ({ ...p, trigger_on: val, response_action: val === 'reaction' ? 'dm' : p.response_action }));
                                }}>
                                    {TRIGGER_ON_OPTIONS.map(t => (<FormControlLabel key={t.value} value={t.value} control={<Radio />} label={t.label} />))}
                                </RadioGroup>

                                <Typography variant="subtitle2" color="primary" sx={{ mt: 1 }}>نوع الرد</Typography>
                                <RadioGroup row value={formData.response_action} onChange={e => setFormData(p => ({ ...p, response_action: e.target.value }))}>
                                    {RESPONSE_ACTIONS.map(a => (
                                        <FormControlLabel key={a.value} value={a.value} control={<Radio />} label={a.label} disabled={formData.trigger_on === 'reaction' && a.value === 'comment'} />
                                    ))}
                                </RadioGroup>

                                {(formData.trigger_on === 'comment' || formData.trigger_on === 'both') && (
                                    <FormControlLabel control={<Switch checked={formData.auto_like} onChange={e => setFormData(p => ({ ...p, auto_like: e.target.checked }))} color="primary" />} label="إعجاب تلقائي على التعليق" />
                                )}
                            </>
                        )}
                        ...
                        <Typography variant="subtitle2" color="primary">
                            {formData.rule_type === 'comment_reply' && (formData.response_action === 'comment' || formData.response_action === 'both') ? 'نص الرد العام (تعليق)' : 'الرد التلقائي'}
                        </Typography>
                        {(formData.rule_type !== 'comment_reply' || formData.response_action === 'comment' || formData.response_action === 'both') && (
                            <TextField label={formData.rule_type === 'comment_reply' ? 'نص التعليق العام' : 'نص الرد'} value={formData.response_text} onChange={e => setFormData(p => ({ ...p, response_text: e.target.value }))} multiline rows={3} required={formData.rule_type !== 'comment_reply'} fullWidth placeholder="اكتب الرد التلقائي هنا..." />
                        )}
                        {formData.rule_type === 'comment_reply' && (formData.response_action === 'dm' || formData.response_action === 'both') && (
                            <TextField label="نص الرسالة الخاصة (DM)" value={formData.dm_text} onChange={e => setFormData(p => ({ ...p, dm_text: e.target.value }))} multiline rows={3} fullWidth placeholder="الرسالة الخاصة التي ستُرسل للمعلق..." />
                        )}
                        <TextField label="فترة التهدئة (بالثواني)" type="number" value={formData.cooldown_seconds} onChange={e => setFormData(p => ({ ...p, cooldown_seconds: parseInt(e.target.value) || 0 }))} helperText={`= ${formatCooldown(formData.cooldown_seconds)}`} fullWidth />
                    </Box>
                </DialogContent>
                <DialogActions>
            ...
            <Dialog open={!!deleteConfirm} onClose={() => setDeleteConfirm(null)} maxWidth="xs">
                ...
                <DialogContent>
                    ...
                    <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>هذا الإجراء لا يمكن التراجع عنه.</Typography>
                </DialogContent>
                <DialogActions>
                    <Button onClick={() => setDeleteConfirm(null)}>إلغاء</Button>
                    <Button variant="contained" color="error" onClick={() => handleDelete(deleteConfirm.id)}>حذف</Button>
                </DialogActions>
            </Dialog>
        </Box>
    );
