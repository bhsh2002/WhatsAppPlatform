...
test('production publishes and consumes the Wa Savana GHCR packages by digest', () => {
    ...
    assert.match(release, /Anonymous access failed for an unverified reason/);
    assert.match(release, /printf '\| Server \| `%s` \|/);
    assert.match(release, /printf '\| Client \| `%s` \|/);
    assert.match(
        productionCompose,
        /ghcr\.io\/bhsh2002\/savana-wa-server@sha256:\$\{WA_SERVER_IMAGE_DIGEST/,
    );
    assert.match(
        productionCompose,
